...
- Nowcast inputs: when `sgwr-nowcast` is enabled, wind speed/direction and cloud-to-ground lightning are fetched for model input even if the wind and lightning map overlays remain hidden. Their display toggles only control rendering; nowcast uses the retained data independently.
